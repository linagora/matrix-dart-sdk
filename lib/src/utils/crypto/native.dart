...
abstract class Cipher {
  ...
  Future<EncryptedFileInfo> encryptStream({
    ...
    final memSize = sizeOf<IntPtr>() +
        key.length +
        initialVector.length +
        maxHashSize +
        memorySizeForHashSizePointer;
    final memNeeded = malloc.call<Uint8>(memSize);

    final intPointer = memNeeded.cast<IntPtr>();
    final keyPointer = memNeeded + sizeOf<IntPtr>();
    final initialVectorPointer = keyPointer + key.length;
    final hashValuePointer = initialVectorPointer + initialVector.length;
    ...
    try {
      ...
      initialVectorPointer
          .asTypedList(initialVector.length)
          .setAll(0, initialVector);
  ...
  String _getHashBase64Encoded(
    ...
    final hashValueBytes =
        Uint8List.fromList(hashValue.asTypedList(maxHashLengthSize));
  ...
  Future<bool> decryptStream({
    ...
    final initialVectorDecoded =
        _base64decodeUnpadded(encryptedFileInfo.initialVector);
    final algorithm = getAlg(keyDecoded.length * 8);

    final memSize = sizeOf<IntPtr>() +
        keyDecoded.length +
        initialVectorDecoded.length +
        maxHashSize +
        memorySizeForHashSizePointer;
    final memNeeded = malloc.call<Uint8>(memSize);
    ...
    try {
      ...
      initialVectorPointer
          .asTypedList(initialVectorDecoded.length)
          .setAll(0, initialVectorDecoded);
      ...
      return hashBase64Encoded ==
          encryptedFileInfo.hashes[messageDigestAlgorithm];
    } catch (e) {
      throw Exception(e);
    } finally {
      malloc.free(memNeeded);
  ...
  Future<void> _decryptAndHashData({
    ...
    await inputStream.forEach((bytes) {
      ...
      final dataPointer = memData + sizeOf<IntPtr>();
      dataPointer.asTypedList(bytes.length).setAll(0, bytes);
      if (EVP_DigestUpdate(mDHashContext, dataPointer, bytes.length) == 0) {
        malloc.free(memData);
