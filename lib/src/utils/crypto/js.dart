...
abstract class Cipher {
  ...
  Future<EncryptedFileInfo> encryptStream({
    ...
    final encryptedBytes =
        await subtle.encrypt(params(initialVector), subtleKey, inputBytes);
    ...
    return EncryptedFileInfo(
      ...
      hashes: {
        messageDigestAlgorithm:
            base64.encode(await sha256(inputBytes)).toUnpaddedBase64(),
      },
    );
  }
  ...
  Future<bool> decryptStream({
    ...
    final keyDecoded =
        base64decodeUnpadded(base64.normalize(encryptedFileInfo.key.key));
    final initialVectorDecoded =
        base64decodeUnpadded(base64.normalize(encryptedFileInfo.initialVector));
    ...
    final inputBytes = Uint8List.fromList(listBytes);
    final subtleKey =
        await importKey('raw', keyDecoded, name, false, ['encrypt']);
    final encryptedBytes = await subtle.encrypt(
      params(initialVectorDecoded),
      subtleKey,
      inputBytes,
    );
    ...
    return base64.encode(await sha256(inputBytes)).toUnpaddedBase64() ==
